...
const IndexPage: NextPage = function () {
  ...
  const fullDate =
    String(thisMonth).length === 2
      ? `${thisYear}-${thisMonth}`
      : `${thisYear}-0${thisMonth}`;

  const [filteredAttendanceThisMonth, setFilteredAttendanceThisMonth] =
    useState<memberType[]>([]);
  const [filteredAttendanceLastMonth, setFilteredAttendanceLastMonth] =
    useState<memberType[]>([]);
  const [filteredFeeThisMonth, setFilteredFeeThisMonth] = useState<
    memberType[]
  >([]);
  const [filteredFeeLastMonth, setFilteredFeeLastMonth] = useState<
    memberType[]
  >([]);
